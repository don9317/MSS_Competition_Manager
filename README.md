# MSS League Scheduler v11.11.77

## v11.11.77 — retain partial schedules and audit unmet guarantees
...
## v11.11.77 — responsive guaranteed-game placement
...
## v11.11.77 — Court Group assignment consistency
...
## v11.11.77 guarantee-first scheduling
...
- Added a bounded same-date edge-swap improvement pass. Swaps preserve every team's nightly game count while reducing premature repeats.
...
- Strengthens the manual Optimize Schedule pass while continuing to yield to the browser.
...
GitHub Pages will launch **index.html**, which is this v11.11.77 application.
For local use, open **START_HERE.html**.

## v11.11.77 — Corrected Diagnostic Build
...
## v11.11.77 — Stable Scheduler + Separate Diagnostic
- `index.html` is the known-working v11.11.42 scheduler code with only the visible version updated to v11.11.77.
...
## v11.11.77 — Schedule Restore / Upgrade Safety
...
- Restoring a schedule replaces only the schedule; teams, divisions, pools, courts, and league setup remain intact.
...
## v11.11.77 — Chronological Immediate Rematch Fix
...
- Only the final emergency fallback may allow an immediate rematch so the 3-games-per-playing-date guarantee is not sacrificed.
...
## v11.11.77 — Audit Detail Expansion
...
## v11.11.77 — Safe Court-Only Utilization
...
## v11.11.77 — Corrected Separate Court + Time Optimizer
- Visible green **Optimize Court + Time Use** button on the Schedule page.
- Visible **BUILD CHECK: v11.11.77** banner confirms the correct `index.html` is live.
- Utilization optimization is a separate action after schedule generation/audit.
- The optimizer may move a game to another legal court and/or time on the same playing date.
- It never changes the matchup, playing date, or number of games.
- Every accepted move must pass the protected Rules Audit.
- Protected rules include nightly/season minimums, maximums, double-booking, time windows, court groups, schedule requests, immediate rematches, opponent rotation, rest, and time-slot fairness.
- Shared-coach conflicts remain warning-level and do not block utilization improvements.
- Rejected moves are restored immediately; a final whole-schedule safety gate restores the original schedule if anything worsens.

## v11.11.77 — Block Court + Time Utilization

- Re-Optimize Schedule is the emphasized scheduling-quality step.
- Optimize Court + Time Use is a separate facility-utilization step.
- Visible progress appears immediately while Court + Time optimization runs.
- The optimizer repacks an entire division/pool/playing-date block instead of moving one game at a time.
- Matchups, playing dates, and game counts never change.
- Multiple bounded packing strategies are attempted per block.
- A block is accepted only if whole-schedule packing improves and no protected Audit category worsens.
- Each team’s within-date game order is preserved to protect chronological opponent rules.
- Final whole-schedule rollback remains in place.
- Completion output now distinguishes full-schedule metrics from single-date Court Calendar metrics and shows per-date before/after results.

## v11.11.77 — Instrumented Single-Date Court + Time Optimization

Purpose: diagnose and stabilize the v54 optimizer hang while keeping the new block-packing approach.

Changes:
- Entire optimizer startup/run/finalization is wrapped in `try/catch/finally`.
- The **Optimize Court + Time Use** button always re-enables, even on an error.
- Exact errors are shown in the Schedule summary and an alert instead of silently hanging.
- Progress updates after every block: `Block X of Y`.
- Browser yields after every block so the interface remains responsive.
- v55 optimizes **one playing date at a time**.
- The optimizer uses the selected Court Calendar date when available; otherwise it uses the first date in the current scope.
- Before/after utilization statistics are calculated for that one date, matching what the Court Calendar displays.
- Final safety gate still restores the original schedule if any protected Audit rule worsens.
- Matchups, playing date, and game counts remain fixed.

## v11.11.77 — Diagnostic Optimization Status

- Permanent Optimization Status panel directly beneath Schedule controls.
- Re-Optimize Schedule immediately shows working status, then COMPLETE/FAILED with before/after Audit failure and warning counts.
- Optimize Court + Time immediately shows working status, Block X of Y progress, a progress bar, live accepted/rejected counts, and a completion timestamp.
- Court + Time diagnostics distinguish: no legal candidate, no packing improvement, and protected-Audit rejection.
- Exact before/after packing, internal empty court-slots, and court blocks are shown for the target playing date.
- Both buttons always re-enable in `finally`.
- Failure/rollback is explicitly shown rather than silently leaving a disabled button.

## v11.11.77 — Global Optimization Stage Fix

This is a surgical execution fix. Scheduling and packing algorithms are unchanged from v11.11.56.

Changes:
- Adds the missing global `setGenerationStage()` utility.
- Re-Optimize Schedule no longer fails with `setGenerationStage is not defined`.
- Optimize Court + Time Use no longer stalls at the initial "Preparing" step for the same reason.
- The Court + Time error path no longer depends on the stage updater succeeding.
- v11.11.56 live progress, diagnostics, before/after metrics, timestamps, rollback protection, and button restoration are retained.

## v11.11.77 — Production Readiness

### Production Preflight
Adds a one-click preflight before official schedule generation. It checks:
- registrations loaded
- playing dates
- missing division assignments
- possible duplicate team names
- division/pool approval
- pool team counts
- Court Group and playing-surface assignments
- valid pool time windows
- per-date pool capacity
- division and shared-resource capacity

The result is a clear **READY TO SCHEDULE** or **DO NOT GENERATE YET** with blocking items and warnings.

### Court + Time
- **Optimize Selected Date** uses the Court Calendar date.
- Fixes the previous selected-date ID issue.
- **Optimize All Playing Dates** loops through every playing date and runs the proven selected-date optimizer with the protected Audit gate.
- Provides a final per-date before/after utilization summary.

### Locked Manual Director Changes
- Schedule Management edits now include **Lock this director change**, ON by default.
- Locked games display `🔒 Locked`.
- Re-Optimize Schedule will not swap teams in a locked game.
- Court/Time optimization will not move a locked game.
- Other unlocked games may still be optimized around the locked game.
- Saving a director change immediately runs a league Audit and reports the resulting hard failures/warnings.
- Lock state is retained in schedule backups, publish copies, history snapshots and undo/restore.

Recommended production workflow:
Import → Preflight → Backup → Generate → Re-Optimize → Audit → Optimize All Dates → Final Audit → Publish.

## v11.11.77 — Simulation Test Lab

Adds an isolated browser-based regression lab that runs the actual production scheduling,
audit, re-optimization, all-dates court/time optimization, preflight, and locked-game logic
against synthetic league data. Simulation mode suppresses IndexedDB writes, so test data
does not overwrite the director's saved league.

Initial suite:
- 4, 5, 6, 7, 8, 9 and 10 team single pools
- 12- and 16-team multi-pool divisions
- shared-coach warning behavior
- high-school 7–10 PM window + all-dates optimization
- locked manual director game preservation through both optimizers
- impossible court-capacity rejection
- impossible schedule-request rejection

Open `SIMULATION_TEST_LAB.html` and select **Run Regression Suite**.

## v11.11.77 — Opponent Rotation Regression Fix
Focused release. Guarantee-first nightly scheduling, Production Preflight, Court/Time optimization, and locked manual changes are unchanged. Re-Optimize adds a conservative same-date/same-pool opponent-coverage repair that cannot worsen protected higher-priority Audit rules and never touches locked games. The Simulation Test Lab runs production Re-Optimize before judging feasible scenarios and shows a failures-only summary.

## v11.11.77 — Chronological Opponent Coverage

This release addresses the key finding from the v60 regression suite: the matchup planner
could prefer first-time opponents, but the time-slot placer could reorder those games and
turn a good matchup plan into a chronological `All opponents before repeats` Audit failure.

Changes:
- Matchup planning gives stronger priority to opponents who still have the greatest unseen-opponent need.
- The safe matchup search bound is increased.
- Time-slot placement now preserves each team's planned opponent sequence within a playing date.
- A new `sequence-relaxed` fallback exists so sequence quality can never override the higher-priority
  3-games-per-date guarantee.
- Immediate-rematch relaxation remains the final emergency fallback.
- The v60 conservative post-repair remains as an additional low-priority cleanup step.
- The Simulation Test Lab visually hides the underlying scheduler UI while tests run, preventing
  deliberately impossible test scenarios from leaving confusing warning cards below the results.

Priority hierarchy remains unchanged:
1. minimum games per team/date
2. no self/team/court double-booking
3. no immediate rematches
4. legal time/court/rest/request rules
5. shared-coach warnings
6. opponent coverage before repeats

## v11.11.77 — Odd-Pool Sequence Repair
Focused on remaining v61 failures S02 (5 teams) and S06 (9 teams). Adds an odd-pool-only chronological opponent exchange that can handle parity-game structures the prior four-distinct-team repair could not. It preserves date/time/court/game counts, locked manual changes, and all higher-priority Audit rules. The 12 v61 passing scenarios remain the regression gate.

## v11.11.77 — Production Re-Optimize Hotfix
Fixes the v62 runtime error `teamPool is not defined`. v63 uses a local, explicit
`poolForTeamSafe(teamId, divisionId)` lookup against the application's existing
`state.pools[divisionId].lists` structure and safely skips teams with no pool assignment.
No scheduling priorities or court/time optimization rules were changed.
The Test Lab adds S15 to execute the production-style 9-team Re-Optimize path.

## v11.11.77 — Production Performance & Browser-Safety Fix

This release addresses the full-league Re-Optimize browser freeze seen with the
97-team / 592-game production rehearsal.

Changes:
- Replaces synchronous odd-pool cleanup with an async bounded version.
- Adds browser yields during opponent candidate evaluation so Chrome can repaint/respond.
- Bounds odd-pool cleanup to 60 candidate checks / ~8 seconds.
- Bounds general opponent cleanup to 80 candidate checks / ~10 seconds.
- Reduces the core full-league repair search limits while retaining the larger limits
  for a selected division/pool.
- Reuses one Audit snapshot per candidate instead of repeatedly running separate
  full-league Audits for score and protection checks.
- Shows live Re-Optimize stage/check-count progress.
- If a cleanup budget is reached, Re-Optimize stops that cleanup safely, keeps only
  accepted improvements, and continues to the final entire-league Audit.
- Scheduling priorities, hard rules, manual locked-game protection, and court/time
  optimization rules are unchanged.

Production goal: bounded, responsive, deadline-safe optimization rather than an
unbounded search for a theoretically perfect opponent sequence.

## v11.11.77 — Deterministic Odd-Pool Repair + Direct Audit Confirmation

Built from v64 to retain the proven responsive production baseline.

Audit:
- Audit buttons now invoke `runAuditWithConfirmation` directly.
- Button visibly changes to Auditing...
- status visibly changes to AUDIT RUNNING before calculation;
- final status is AUDIT COMPLETE with timestamp, runtime, FAIL and WARN counts.

Opponent sequencing:
- identifies only odd pools currently failing Immediate Rematches / All Opponents Before Repeats;
- constructs deterministic circle-method round-robin rounds;
- attempts to map complete round combinations onto each pool's existing date/slot framework;
- requires exact existing per-team nightly game-count vectors;
- never changes dates, times, courts, or locked manual games;
- accepts a complete pool rebuild only when league-wide opponent sequencing improves
  and no protected hard Audit rule worsens;
- bounded to about 10 seconds total and yields to Chrome.

## v11.11.77 SAFE — v67 Baseline + Hard Regression Gate
...
## v11.11.77 — Surgical Opponent Repair
Built from v11.11.69 SAFE. Generate Schedule is unchanged.
Re-Optimize targets only pools currently failing Immediate Rematches / All Opponents Before Repeats.
The new repair searches whole-pool, degree-preserving two-edge switches while leaving every game slot fixed.
Each candidate receives a full protected Audit check. Any worsening of self-match, team/court double-booking,
division windows, court groups, team requests, minimum games/date, season minimum, maximum games/date,
or rest/back-to-back rules is rejected. Search is browser-yielding and time/candidate bounded.

## v11.11.77 — Opponent Balance
Immediate-rematch definition remains chronological across dates. Adds Opponent Balance audit and bounded 3-game-cycle surgical repairs. Generate Schedule is unchanged. Court/time optimization remains a separate final step.

## v11.11.77 — Odd-Pool Diagnostic
Diagnostic release. Scheduling and re-optimization behavior is unchanged from v71. Adds a generated-schedule comparison for every odd-team pool: actual vs expected games/date, teams receiving parity-extra games, opponents of those extra-game teams, repeated pairings, and per-team opponent frequencies. Intended to compare the working 7-team pool against failing 9-team and 5-team pools before changing generator logic.
\n## v11.11.77 — Same-Day Opponent Uniqueness\nGenerator now tracks pairings within each playing date. Same-day duplicate opponents are the strongest opponent preference, followed by unseen season opponents and least-played opponents. This is preference-only: exact nightly game guarantees remain primary. Immediate-rematch definition remains chronological across dates. v72 odd-pool diagnostics retained.\n
## v11.11.77 — Strict Parity-Extra Opponent Uniqueness
The exact nightly degree planner now runs a strict first pass that forbids using the same pairing twice on a playing date. If no exact degree solution exists, it falls back to the guarantee-first planner so the 3-games/team/date contract is never sacrificed. Season unseen/least-used opponent preferences and chronological immediate-rematch rules remain in force. Court/time optimization remains separate.


## v11.11.77
Opponent balance is now included in the protected rollback vector for all opponent cleanup stages. A candidate repair is rejected if it increases Opponent Balance violations. Generator behavior remains the v74 parity-uniqueness baseline.
