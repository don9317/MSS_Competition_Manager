# MSS League Scheduler v11.11.52

## v11.11.52 — retain partial schedules and audit unmet guarantees
...
## v11.11.52 — responsive guaranteed-game placement
...
## v11.11.52 — Court Group assignment consistency
...
## v11.11.52 guarantee-first scheduling
...
GitHub Pages will launch **index.html**, which is this v11.11.52 application.
For local use, open **START_HERE.html**.

## v11.11.52 — Corrected Diagnostic Build
...
## v11.11.52 — Stable Scheduler + Separate Diagnostic
- `index.html` is the known-working v11.11.42 scheduler code with only the visible version updated to v11.11.52.
...
## v11.11.52 — Schedule Restore / Upgrade Safety
...
## v11.11.52 — Chronological Immediate Rematch Fix
...
## v11.11.52 — Audit Detail Expansion
...
## v11.11.52 — Safe Court-Only Utilization
...
## v11.11.52 — Automatic Three-Stage Scheduling + Safe Time/Court Compaction

Generate Schedule now performs the complete director workflow automatically:

1. **Build Games** — preserve nightly minimums, season counts, legal resources and placement rules.
2. **Improve Rules** — bounded repair for immediate rematches, opponent rotation and rest issues.
3. **Compact Court Use** — move the exact same matchup to an earlier/later legal start and/or another legal court on the SAME playing date when doing so improves court packing.

Safety rules for Stage 3:
- Opponents never change.
- Playing date never changes.
- Game count never changes.
- Destination must be a legal division/pool court/time slot.
- No court/time or team/time double-booking may be created.
- Schedule requests, rest, chronological immediate rematches, and all-opponents-before-repeat counts may not worsen.
- A complete Rules Audit runs after compaction. If any Audit rule worsens, the entire court-compaction stage is automatically rolled back to the exact pre-compaction schedule.

Director simplification:
- Re-Optimize Schedule and Optimize Court Use buttons are removed from the normal workflow.
- Generate Schedule automatically performs all three stages.
- Live progress appears in the Schedule summary during court compaction, including safe-slot checks, accepted moves and current packing percentage.
- Completion summary reports packing percentage, internal empty court-slots and court blocks before/after.

Court Calendar metrics from v11.11.51 are retained.
